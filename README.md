...
The four overlapping views answer distinct questions: who is in the Inner Circle, who is overdue for reconnection, who has been contacted recently, and who is a deliberate Target. The same action views can focus the center map without hiding network context. Selecting a person in the sidebar, graph, or search opens a detail panel designed to support the next conversation.
...
Known team dynamics, responsibilities, current pressures, and priorities.

### What they care about

- A known goal or preference

### Remember

- An important commitment or context item
...
An optional `## Conversation Prep` section can contain four recognized level-three headings:

- `### Their world`: team dynamics, responsibilities, current pressures, and priorities.
- `### What they care about`: known goals, motivations, interests, and preferences.
- `### Remember`: commitments, personal context, sensitivities, and details worth retaining.
- `### Next conversation`: questions, topics, and matters to follow up on.

All four blocks are optional. **Next conversation** receives the strongest emphasis in the detail panel; the durable-context blocks are compact disclosures. When the entire section is absent, **Copy template** places the empty schema on the clipboard but never edits the source file.

Record known facts and clearly framed personal observations. Do not present guesses, gossip, or unsupported assumptions about a person as fact, and avoid storing sensitive information that is not necessary for a better professional conversation.

## Derived views

The sidebar and map views deliberately overlap; a person can appear in several of them.
...
The center-map controls provide **All**, **Inner Circle**, **Reconnect**, **Recent**, and **Targets**. Choosing a view dims nonmatching people and unrelated links rather than hiding them, preserving the larger relationship context. Matching people receive a view-specific halo and a restrained breathing effect; the effect becomes static when the operating system requests reduced motion. Hover and selection remain visually dominant.
...
This keeps targets connected while preserving meaningful topology. The canvas graph supports pan, zoom, hover metadata, selection, and person-node dragging; names, a relationship-strength radius, an independent strategic-relevance rim, and temporary group halos communicate different signals. Dragged coordinates persist only in this browser under the versioned `orbit.graph-layout.v1` key. **Reset layout** clears those coordinates, releases fixed nodes, and returns to the automatic force layout. Select the same people through the semantic sidebar or keyboard command palette when canvas interaction is not appropriate.
...
Orbit uses a warm, light, system-UI productivity-tool palette: an off-white canvas, near-black text, quiet gray rails and dividers, terracotta/high, sage/medium, and neutral/low relevance markers, plus a deep ink/teal selection state. Action-filter halos use a separate small palette so they never replace strategic-relevance meaning. CSS tokens define the color, radius (4/6/8 px), shadow, and motion scales (120/220/380 ms).

The layout prioritizes desktop widths around 1280–1800 px: a compact relationship sidebar, flexible graph field, and in-flow detail panel. On narrower screens the detail panel becomes an overlay. The graph is intentionally not a dashboard: there are no gradients, glass effects, decorative blobs, nested card grids, or attention-seeking animations. Motion is limited to structural feedback and the slow selected-group halo; `prefers-reduced-motion` makes that halo static, removes graph entrance movement, and reduces spatial panel transitions to restrained opacity changes. Visible focus rings remain high contrast.

## Limitations

Orbit is intentionally read-only, desktop-first, local-only, and repository-scoped. It has no accounts, authentication, hosted database, cloud sync, CRUD forms, file watcher, or polling. Markdown changes are visible after Refresh or a normal browser reload, not automatically. The application reads only direct Markdown files in `data/people/`; it does not ingest email, calendars, LinkedIn, or external relationship data. Dragged coordinates are specific to one browser profile and do not sync between browsers or computers.
...
These V2 capabilities are deliberately deferred and are absent from the source and dependencies: shortest warm-introduction paths; richer typed edges; relationship timelines; strength-by-relevance matrices; organization clustering; one-person ego views; collapsed graph clusters; alternate `Color by` modes; automated stale-relationship recommendations; interaction analytics; calendar or email ingestion; local-LLM features; semantic search; natural-language queries; suggested follow-ups; organization mapping; and opportunity mapping.
